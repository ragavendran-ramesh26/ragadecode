...
const API_URL = 'https://genuine-compassion-eb21be0109.strapiapp.com/api/tourism-travel-trips?populate=coverimage&sort[0]=id:desc';
...
(async () => {
  try {
    ...
    for (const article of data) {
      ...
      else if (category.includes('automobile')) sections.automobile.push(html); // ✅ Add this
      else if (category.includes('tourism-travel-trip')) sections.travel.push(html); // ✅ Add this
...
`;
    ...
    console.log(`✅ tourism-travel-trips.html generated with ${data.length} articles`);

  } catch (err) {
    console.error('❌ Failed to generate tourism-travel-trips page:', err.message);
  }
})();
